...
int sl_eps2_i2c_write(sl_eps2_config_t config, uint8_t *data, uint16_t len)
{
	...
	if (fd < 0) {
		perror("EPS: Could not open i2c device");
		return -1;
	}

	if (ioctl(fd, I2C_SLAVE, SL_EPS2_I2C_SLAVE_ADR) < 0) {
		perror("EPS: Failed to acquire bus access and/or talk to slave");
		close(fd);
		return -1;
	}

	if (write(fd, data, len) != len) {
		perror("EPS: Could not write to i2c device");
		close(fd);
		return -1;
	}
...
int sl_eps2_i2c_read(sl_eps2_config_t config, uint8_t *data, uint16_t len)
{
	...
	if (read(fd, data, len) != len) {
		perror("EPS: Could not read to i2c device");
		close(fd);
		return -1;
	}
